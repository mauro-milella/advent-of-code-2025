...
--- Retrieve all the neighbor_char that are alive and can be removed;
--- see `living_cell`
local function get_removables(map, nrows, ncols, x, neighbor_char)
    local removables = {}
    
    for i = 1, nrows do
        for j = 1, ncols do
            if living_cell(map, i, j, 4, '@') then
                table.insert(removables, {i,j})
            end
        end
    end
...
while true do
    local removables = get_removables(map, nrows, ncols, 4, '@')
    local nremovables = #removables

    if nremovables == 0 then
        break
    else
        ans = ans + nremovables

        -- update map for the next iteration
        for ci = 1, nremovables do
            local _coordinates = removables[ci]
            local i, j = _coordinates[1], _coordinates[2]
            map[i][j] = '.'
        end
    end
end
